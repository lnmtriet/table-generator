...
const API_URL = 'https://raw.githubusercontent.com/lnmtriet/table-generator/master/data/snippet.json';
...
class SnippetProvider {
	...
	getSuggestions(options) {
		...
		var matched = this.colRangeMatching(prefix);
		if (!!matched){
			return matched;
		}
	...
	colRangeMatching(prefix){
		var rangeTypes = ["fico", "ltv", "cltv", "term", "la"];
		for (var i = 0 ; i < rangeTypes.length; i++){
			var patt = new RegExp("^@" + rangeTypes[i] + "\\([0-9]+");
			if (patt.test(prefix)) {
				return this.generateColRange(prefix, rangeTypes[i]);
			}
		}
	}
	generateColRange(prefix, type) {
		var numberOfCols = prefix.substring(type.length + 2);
		...
		return [{
			...
			snippet: mySnippet,
			description: "Define a ranges of fico/ltv/cltv/term/la with " + numberOfCols +" rows/columns",
			...
			type: 'snippet',
			rightLabelHTML: '<span class="aab-right-label">Range Condition</span>' // look in /styles/atom-slds.less
		}];
	}
